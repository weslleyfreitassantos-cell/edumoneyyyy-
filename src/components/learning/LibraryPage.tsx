...
const recommendations = [
  { subject: 'História', title: 'Uma breve história do tempo', author: 'Stephen Hawking', isbn: '9788535905409', note: 'Uma leitura acessível para conhecer ideias que mudaram a ciência.', query: 'Uma breve história do tempo Stephen Hawking' },
  { subject: 'História', title: 'O povo brasileiro', author: 'Darcy Ribeiro', isbn: '9788526022574', note: 'Uma introdução marcante à formação histórica e cultural do Brasil.', query: 'O povo brasileiro Darcy Ribeiro' },
  { subject: 'Ciências', title: 'O mundo assombrado pelos demônios', author: 'Carl Sagan', isbn: '9788535908349', note: 'Pensamento crítico e curiosidade científica em uma leitura envolvente.', query: 'O mundo assombrado pelos demônios Carl Sagan' },
  { subject: 'Matemática', title: 'O homem que calculava', author: 'Malba Tahan', isbn: '9788504014464', note: 'Matemática apresentada por meio de histórias e desafios.', query: 'O homem que calculava Malba Tahan' },
  { subject: 'Português', title: 'Gramática em textos', author: 'Ulisses Infante', isbn: '9788526284255', note: 'Apoio prático para estudar língua portuguesa e produção textual.', query: 'Gramática em textos Ulisses Infante' },
  { subject: 'Literatura', title: 'Dom Casmurro', author: 'Machado de Assis', isbn: '9788535910663', note: 'Um clássico brasileiro para ler, interpretar e discutir.', query: 'Dom Casmurro Machado de Assis livro' },
];
...
export default function LibraryPage() {
  ...
  return <div className="space-y-6">
    ...
    <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">{visible.map((book) => <article key={book.title} className="overflow-hidden rounded-xl border border-[#dfe3e8] bg-white shadow-sm dark:border-slate-700 dark:bg-slate-900"><div className="flex gap-4 p-5"><div className="h-40 w-28 shrink-0 overflow-hidden rounded-lg bg-blue-50 dark:bg-blue-950/40"><img src={`https://covers.openlibrary.org/isbn/${book.isbn}-M.jpg`} alt={`Capa de ${book.title}`} className="h-full w-full object-cover" onError={(event) => { event.currentTarget.style.display = 'none'; }} /><div className="grid h-full place-items-center text-[#005bbf]"><BookOpen className="h-8 w-8" aria-hidden="true" /></div></div><div className="min-w-0 flex-1"><div className="flex items-start justify-between gap-2"><span className="rounded-full bg-slate-100 px-2 py-1 text-[11px] font-bold text-slate-600 dark:bg-slate-800 dark:text-slate-300">{book.subject}</span></div><h3 className="mt-3 font-bold text-[#181c20] dark:text-white">{book.title}</h3><p className="mt-1 text-sm font-semibold text-[#005bbf]">{book.author}</p></div></div><div className="border-t border-slate-100 px-5 pb-5 pt-4 dark:border-slate-800"><p className="min-h-12 text-sm leading-6 text-[#727785] dark:text-slate-400">{book.note}</p><div className="mt-4 flex items-center gap-1 text-amber-500" aria-label="Recomendado"><Star className="h-4 w-4 fill-current" /><span className="text-xs font-semibold text-slate-500">Recomendado para estudo</span></div><div className="mt-5 flex flex-wrap gap-2"><a target="_blank" rel="noreferrer" href={storeUrl('amazon', book.query)} className="inline-flex items-center gap-2 rounded-lg bg-[#005bbf] px-3 py-2 text-xs font-bold text-white"><ExternalLink className="h-4 w-4" />Ver na Amazon</a><a target="_blank" rel="noreferrer" href={storeUrl('mercadoLivre', book.query)} className="inline-flex items-center gap-2 rounded-lg border border-[#cfd7e6] px-3 py-2 text-xs font-bold text-[#005bbf]"><ExternalLink className="h-4 w-4" />Ver no Mercado Livre</a></div></div></article>)}</section>
