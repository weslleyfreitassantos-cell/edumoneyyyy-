              ...
              ([termName, subjects]) => {
                ...
                return (
                  ...
                  >
                    ...
                    <div className="divide-y divide-[#edf0f3] dark:divide-slate-800 sm:hidden print:hidden">
                      {subjects.map((subject) => (
                        <div key={subject.key} className="space-y-3 px-4 py-4">
                          <div className="flex items-start justify-between gap-3">
                            <div className="min-w-0">
                              <p className="break-words font-semibold text-[#181c20] dark:text-slate-100">
                                {subject.subjectName}
                              </p>
                              <p className="mt-1 break-words text-sm text-[#727785] dark:text-slate-400">
                                {subject.teacherName}
                              </p>
                            </div>
                            {!subject.isClosed || subject.resultStatus === 'PENDING' ? (
                              <span className="inline-flex w-fit max-w-[55%] shrink-0 break-words rounded-full border border-amber-200 bg-amber-50 px-2.5 py-1 text-left text-xs font-semibold text-amber-700 dark:border-amber-900/60 dark:bg-amber-950/30 dark:text-amber-300">
                                Resultado ainda não fechado
                              </span>
                            ) : (
                              <span
                                className={`inline-flex w-fit max-w-[55%] shrink-0 break-words rounded-full border px-2.5 py-1 text-left text-xs font-semibold ${getResultBadgeClass(subject.resultStatus)}`}
                              >
                                {getResultStatusLabel(subject.resultStatus)}
                              </span>
                            )}
