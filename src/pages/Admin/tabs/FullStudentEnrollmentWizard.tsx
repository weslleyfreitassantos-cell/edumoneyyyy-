...
const inputClass =
  'mt-1 w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm text-slate-900 outline-none transition focus:border-blue-600 focus:ring-2 focus:ring-blue-100 dark:border-slate-600 dark:bg-slate-950 dark:text-slate-100 dark:placeholder:text-slate-500 dark:focus:border-blue-400 dark:focus:ring-blue-400/20';
...
export default function FullStudentEnrollmentWizard({
  ...
  const isExistingStudentMode = mode === 'edit' || mode === 'enroll';
  ...
  const [draft, setDraft] = useState<FullStudentEnrollmentDraft>(() =>
    createDraft(
      isExistingStudentMode ? '' : firstYear,
      isExistingStudentMode ? '' : firstClassId,
    ),
  );
  ...
  const editorQuery = useStudentEditorData(
    institutionId,
    isExistingStudentMode ? studentId ?? null : null,
  );
  ...
  useEffect(() => {
    if (!isExistingStudentMode && !draft.academic_year_id && firstYear) {
      setDraft((current) => ({
        ...current,
  ...
  }, [draft.academic_year_id, firstClassId, firstYear, isExistingStudentMode]);
  ...
  async function next() {
    ...
    if (step === 0 && !isExistingStudentMode) {
  ...
  function renderAddress() {
    return (
      <div className="grid gap-4 md:grid-cols-2">
        ...
        <label className="flex items-center gap-2 self-end pb-2 text-sm text-slate-700 dark:text-slate-200"><input type="checkbox" checked={draft.address.rural_zone} onChange={(event) => updateAddress('rural_zone', event.target.checked)} /> Zona rural</label>
      </div>
    );
  }
  ...
  function renderGuardians() {
    return (
      <div className="space-y-4">
        <p className="text-sm text-slate-600 dark:text-slate-400">Associe um ou mais responsáveis. O principal será usado como contato prioritário.</p>
        {draft.guardians.map((guardian, index) => {
          ...
          return (
            <div key={`${index}-${guardian.profile_id}`} className="rounded-lg border border-slate-200 p-4 dark:border-slate-700">
              <div className="mb-3 flex items-center justify-between gap-3"><strong className="text-sm text-slate-800 dark:text-slate-100">Responsável {index + 1}</strong>{draft.guardians.length > 1 ? <button type="button" className="text-sm text-red-600 dark:text-red-300" onClick={() => removeGuardian(index)}>Remover</button> : null}</div>
              ...
              {matchingGuardian ? <div className="mt-3 rounded-lg border border-amber-200 bg-amber-50 p-3 text-sm text-amber-900 dark:border-amber-900/70 dark:bg-amber-950/40 dark:text-amber-200"><p>Já existe um responsável com este e-mail: <strong>{matchingGuardian.profile?.full_name ?? matchingGuardian.profile?.email}</strong>.</p><button type="button" className="mt-2 rounded-lg border border-amber-700 px-3 py-2 font-semibold dark:border-amber-500 dark:text-amber-300" onClick={() => updateGuardian(index, { mode: 'existing', profile_id: matchingGuardian.profile_id })}>Usar cadastro existente</button></div> : null}
              <label className="mt-3 flex items-center gap-2 text-sm text-slate-700 dark:text-slate-200"><input type="radio" name="primary-guardian" checked={guardian.is_primary} onChange={() => setPrimaryGuardian(index)} /> Responsável principal</label>
            </div>
          );
        })}
  ...
  function renderHealth() {
    ...
    return <div className="space-y-4"><div className="rounded-lg border border-amber-200 bg-amber-50 p-3 text-sm text-amber-800 dark:border-amber-900/70 dark:bg-amber-950/40 dark:text-amber-200">Dados de saude sao sensiveis e ficam restritos a ADMIN, DIRECTOR e SECRETARY.</div><div className="grid gap-4 md:grid-cols-2"><Field label="Alergias"><textarea className={inputClass} value={health.allergies} onChange={(event) => update('allergies', event.target.value)} /></Field><Field label="Condicoes de saude"><textarea className={inputClass} value={health.health_conditions} onChange={(event) => update('health_conditions', event.target.value)} /></Field><Field label="Medicacao de emergencia"><textarea className={inputClass} value={health.emergency_medication} onChange={(event) => update('emergency_medication', event.target.value)} /></Field><Field label="Deficiencia ou necessidade"><textarea className={inputClass} value={health.disability} onChange={(event) => update('disability', event.target.value)} /></Field><Field label="Cuidados na escola"><textarea className={inputClass} value={health.school_care_notes} onChange={(event) => update('school_care_notes', event.target.value)} /></Field></div><div className="flex flex-wrap gap-4 text-sm text-slate-700 dark:text-slate-200"><label className="flex items-center gap-2"><input type="checkbox" checked={health.autism} onChange={(event) => update('autism', event.target.checked)} /> TEA</label><label className="flex items-center gap-2"><input type="checkbox" checked={health.giftedness} onChange={(event) => update('giftedness', event.target.checked)} /> Altas habilidades</label><label className="flex items-center gap-2"><input type="checkbox" checked={health.needs_special_education} onChange={(event) => update('needs_special_education', event.target.checked)} /> Atendimento educacional especializado</label></div></div>;
  }

  function renderDocuments() {
    return <div className="space-y-3"><p className="text-sm text-slate-600 dark:text-slate-400">Esta etapa registra a pendência documental. O envio de arquivos será ativado somente com armazenamento privado e política validada.</p>{draft.documents.map((document, index) => <div key={document.document_type} className="grid gap-2 rounded-lg border border-slate-200 p-3 dark:border-slate-700 md:grid-cols-[1fr_180px_1fr]"><span className="self-center text-sm font-medium text-slate-700 dark:text-slate-200">{document.document_type}</span><select className={inputClass} value={document.status} onChange={(event) => setDraft((current) => ({ ...current, documents: current.documents.map((item, itemIndex) => itemIndex === index ? { ...item, status: event.target.value as StudentDocumentDraft['status'] } : item) }))}><option value="PENDING">Pendente</option><option value="DELIVERED">Entregue</option><option value="VALIDATED">Validado</option><option value="DISPENSED">Dispensado</option></select><input className={inputClass} placeholder="Observação" value={document.notes} onChange={(event) => setDraft((current) => ({ ...current, documents: current.documents.map((item, itemIndex) => itemIndex === index ? { ...item, notes: event.target.value } : item) }))} /></div>)}</div>;
  }
  ...
  function renderReview() {
    ...
    return <div className="space-y-4 text-sm text-slate-700 dark:text-slate-200"><div className="grid gap-3 md:grid-cols-2"><div><strong>Aluno</strong><p>{draft.identity.full_name}</p><p>{draft.identity.email}</p><p>{draft.identity.birth_date}</p></div><div><strong>Matrícula</strong><p>{year?.name ?? 'Ano não selecionado'}</p><p>{classRecord?.name ?? 'Turma não selecionada'}</p><p>{draft.enrolled_at}</p></div><div><strong>Responsável principal</strong><p>{primary?.full_name || (primary?.profile_id ? 'Responsável cadastrado' : 'Não informado')}</p><p>{primary?.relationship}</p></div><div><strong>Documentos pendentes</strong><p>{draft.documents.filter((document) => document.status === 'PENDING').length}</p></div></div><div className="rounded-lg border border-blue-100 bg-blue-50 p-3 text-blue-900 dark:border-blue-900/70 dark:bg-blue-950/40 dark:text-blue-200">Ao confirmar, o aluno, os responsáveis, os dados complementares e a matrícula serão persistidos. O acesso por e-mail será criado conforme o fluxo de autenticação.</div></div>;
  }
  ...
  if (isExistingStudentMode && editorQuery.isLoading) {
    return (
      <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/50 p-3" role="dialog" aria-modal="true" aria-label="Editar aluno">
        <div className="w-full max-w-md rounded-xl bg-white p-6 shadow-2xl dark:border dark:border-slate-700 dark:bg-slate-900">
          <p className="text-sm font-semibold text-slate-800 dark:text-slate-100">Carregando dados completos do aluno...</p>
        </div>
      </div>
    );
  }

  if (isExistingStudentMode && editorQuery.isError) {
    return (
      <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/50 p-3" role="dialog" aria-modal="true" aria-label="Editar aluno">
        <div className="w-full max-w-md rounded-xl bg-white p-6 shadow-2xl dark:border dark:border-slate-700 dark:bg-slate-900">
          <ErrorBox message={getUserFacingErrorMessage(editorQuery.error, 'Não foi possível carregar os dados do aluno.')} />
          <div className="mt-4 flex justify-end"><button type="button" className="rounded-lg border border-slate-300 px-4 py-2 text-sm font-semibold text-slate-700 dark:border-slate-600 dark:text-slate-200 dark:hover:bg-slate-800" onClick={onClose}>Fechar</button></div>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/50 p-3" role="dialog" aria-modal="true" aria-label={isExistingStudentMode ? (mode === 'enroll' ? 'Matricular aluno' : 'Editar aluno') : 'Matrícula completa de aluno'}>
      <div className="flex max-h-[94vh] w-full max-w-6xl flex-col overflow-hidden rounded-xl bg-white shadow-2xl dark:border dark:border-slate-700 dark:bg-slate-900">
        <div className="flex items-start justify-between border-b border-slate-200 px-5 py-4 dark:border-slate-700"><div><p className="text-xs font-semibold uppercase tracking-wide text-blue-700 dark:text-blue-300">{isExistingStudentMode ? (mode === 'enroll' ? 'Matricular aluno' : 'Editar aluno') : 'Novo aluno'}</p><h2 className="mt-1 text-xl font-bold text-slate-900 dark:text-white">{mode === 'enroll' ? 'Concluir matrícula' : isEditMode ? 'Editar cadastro completo' : 'Matrícula completa'}</h2><p className="mt-1 text-sm text-slate-600 dark:text-slate-400">{mode === 'enroll' ? 'Revise os dados e escolha a turma para concluir a matrícula deste aluno.' : isEditMode ? 'Atualize todos os dados cadastrais, escolares e da matrícula.' : 'Cadastro detalhado com vínculos, documentos e revisão antes de confirmar.'}</p></div><button type="button" className="rounded-lg border border-slate-300 px-3 py-2 text-slate-700 dark:border-slate-600 dark:text-slate-200 dark:hover:bg-slate-800" onClick={onClose} aria-label="Fechar">X</button></div>
        <div className="overflow-y-auto px-5 py-4 dark:bg-slate-900"><div className="mb-5 grid grid-cols-2 gap-2 md:grid-cols-8">{steps.map((label, index) => <div key={label} className={`rounded-lg px-2 py-2 text-center text-xs font-semibold ${index === step ? 'bg-blue-700 text-white' : index < step ? 'bg-blue-50 text-blue-700 dark:bg-blue-950/40 dark:text-blue-300' : 'bg-slate-100 text-slate-500 dark:bg-slate-800 dark:text-slate-300'}`}><span className="block">{index + 1}</span>{label}</div>)}</div><div className="mb-4"><ErrorBox message={error} />{duplicates.length > 0 ? <div className="mt-3 rounded-lg border border-amber-200 bg-amber-50 p-3 text-sm text-amber-900 dark:border-amber-900/70 dark:bg-amber-950/40 dark:text-amber-200"><strong>Possíveis duplicidades</strong>{duplicates.map((candidate) => <div key={candidate.id} className="mt-2 rounded-lg border border-amber-300 bg-white/70 p-3 dark:border-amber-800 dark:bg-slate-900/70"><p className="font-semibold">{candidate.full_name}</p><p className="mt-1">Matrícula {candidate.registration_number} - {candidate.birth_date}</p>{onUseExistingStudent ? <button type="button" className="mt-2 rounded-lg border border-blue-600 px-3 py-2 font-semibold text-blue-700 dark:border-blue-500 dark:text-blue-300" onClick={() => onUseExistingStudent(candidate.id)}>Usar este cadastro</button> : null}</div>)}<div className="mt-3 flex flex-wrap gap-2"><button type="button" className="rounded-lg border border-slate-400 bg-white px-3 py-2 font-semibold text-slate-700 dark:border-slate-600 dark:bg-slate-800 dark:text-slate-200" onClick={() => setDuplicates([])}>Voltar e revisar</button><button type="button" className="rounded-lg bg-amber-700 px-3 py-2 font-semibold text-white" onClick={continueAfterDuplicateCheck}>Continuar mesmo assim</button></div></div> : null}</div><div className="min-h-[300px]">{renderStep()}</div></div>
        <div className="flex flex-wrap items-center justify-between gap-3 border-t border-slate-200 px-5 py-4 dark:border-slate-700"><button type="button" className="rounded-lg border border-slate-300 px-4 py-2 text-sm font-semibold text-slate-700 dark:border-slate-600 dark:text-slate-200 dark:hover:bg-slate-800" onClick={step === 0 ? onClose : () => { setError(null); setStep((current) => current - 1); }}>{step === 0 ? 'Cancelar' : 'Voltar'}</button><div className="flex gap-2"><span className="self-center text-xs text-slate-500 dark:text-slate-400">Etapa {step + 1} de {steps.length}</span>{step < steps.length - 1 ? <button type="button" className="rounded-lg bg-blue-700 px-4 py-2 text-sm font-semibold text-white disabled:opacity-60" onClick={next} disabled={isCheckingDuplicates}>{isCheckingDuplicates ? 'Verificando...' : 'Continuar'}</button> : <button type="button" className="rounded-lg bg-green-700 px-4 py-2 text-sm font-semibold text-white disabled:opacity-60" onClick={submit} disabled={createMutation.isPending || updateMutation.isPending}>{createMutation.isPending || updateMutation.isPending ? 'Salvando...' : mode === 'enroll' ? 'Confirmar matrícula' : isEditMode ? 'Salvar alterações' : 'Confirmar matrícula'}</button>}</div></div>
      </div>
    </div>
  );
}
