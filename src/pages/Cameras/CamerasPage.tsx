...
import { Camera, CheckCircle2, Edit3, Gauge, Plus, Power, PowerOff, Radio, RefreshCw, Server, Trash2, WifiOff, X } from 'lucide-react';
...
function CameraForm({ institutionId, camera, gatewayIdOverride, onClose, onSaved, gateways }: CameraFormProps) {
  const [form, setForm] = useState(() => camera
    ? initialCameraForm(camera, institutionId, gatewayIdOverride, gateways)
    : readNewCameraDraft(institutionId) ?? initialCameraForm(null, institutionId, gatewayIdOverride, gateways));
  ...
  useEffect(() => {
    if (camera) return;
    writeNewCameraDraft(institutionId, form);
  }, [camera, form, institutionId]);
...
function relayLabel(gateway: CameraGateway): string {
  if (!gateway.relayConfigured) return 'Relay HTTPS não configurado';
  if (gateway.relayStatus === 'ONLINE') return 'Relay HTTPS online';
  if (gateway.relayStatus === 'OFFLINE') return 'Relay HTTPS offline';
  return 'Relay HTTPS não conectado';
}
...
export function CamerasPage() {
  ...
  const [formCamera, setFormCamera] = useState<DirectorCamera | null | undefined>(() => currentInstitutionId && hasNewCameraDraft(currentInstitutionId) ? null : undefined);
  const [playerCamera, setPlayerCameraState] = useState<DirectorCamera | null>(null);
  const [streamSession, setStreamSession] = useState<Awaited<ReturnType<typeof cameraService.createStreamSession>> | null>(null);
  ...
  useEffect(() => {
    if (formCamera !== undefined || !currentInstitutionId) return;
    if (hasNewCameraDraft(currentInstitutionId)) {
      setFormCamera(null);
    }
  }, [currentInstitutionId, formCamera]);
  ...
  return (
    <main id="app-main-content" className="min-w-0 flex-1 bg-[#f3f6fb] p-4 sm:p-6 dark:bg-slate-950">
      <div className="mx-auto max-w-7xl">
        ...
        <section className="min-w-0 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm sm:p-6 dark:border-slate-800 dark:bg-slate-900">
          <div className="mb-5 flex flex-col gap-3 border-b border-slate-200 pb-4 sm:flex-row sm:items-center sm:justify-between dark:border-slate-800"><div className="min-w-0"><h2 className="text-lg font-bold text-slate-900 dark:text-white">Dispositivos da instituição</h2><p className="break-words text-sm text-slate-500 dark:text-slate-400">Sem áudio, gravação, download ou acesso para responsáveis.</p></div><div className="flex min-w-0 items-center gap-2 sm:shrink-0"><select aria-label="Filtrar status" value={filter} onChange={(event) => setFilter(event.target.value as CameraFilter)} className="min-w-0 flex-1 rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm dark:border-slate-700 dark:bg-slate-950 dark:text-white sm:flex-none"><option value="ALL">Todos</option><option value="ONLINE">Online</option><option value="OFFLINE">Offline</option><option value="UNKNOWN">Não conectados</option></select><button type="button" onClick={() => void camerasQuery.refetch()} className="rounded-lg border border-slate-300 p-2 text-slate-600 hover:bg-slate-100 dark:border-slate-700 dark:text-slate-300 dark:hover:bg-slate-800" aria-label="Atualizar câmeras"><RefreshCw className="h-4 w-4" /></button></div></div>

          {camerasQuery.isLoading ? <div className="grid min-h-48 place-items-center text-sm text-slate-500">Carregando câmeras...</div> : camerasQuery.isError ? <div className="grid min-h-48 place-items-center text-center"><WifiOff className="mb-3 h-8 w-8 text-red-500" /><p className="font-semibold text-slate-800 dark:text-slate-200">Não foi possível carregar as câmeras.</p><button type="button" onClick={() => void camerasQuery.refetch()} className="mt-3 rounded-lg border border-slate-300 px-3 py-2 text-sm font-semibold dark:border-slate-700 dark:text-slate-200">Tentar novamente</button></div> : filteredCameras.length === 0 ? <div className="grid min-h-48 place-items-center rounded-xl border border-dashed border-slate-300 px-6 text-center dark:border-slate-700"><Server className="mb-3 h-9 w-9 text-slate-400" /><p className="font-semibold text-slate-800 dark:text-slate-200">{cameras.length ? 'Nenhuma câmera corresponde ao filtro.' : 'Nenhuma câmera cadastrada.'}</p><p className="mt-1 text-sm text-slate-500 dark:text-slate-400">Cadastre o primeiro dispositivo para preparar o gateway local.</p>{!cameras.length && <button type="button" onClick={() => setFormCamera(null)} className="mt-4 inline-flex items-center gap-2 rounded-lg bg-blue-700 px-4 py-2 text-sm font-bold text-white"><Plus className="h-4 w-4" />Adicionar primeira câmera</button>}</div> : <div className="grid gap-4 lg:grid-cols-2">{filteredCameras.map((camera) => { const gateway = gateways.find((item) => item.id === camera.gatewayId); return <article key={camera.id} className="min-w-0 rounded-xl border border-slate-200 p-4 dark:border-slate-700"><div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between"><div className="flex min-w-0 items-start gap-3"><span className="rounded-lg bg-blue-50 p-2 text-blue-700 dark:bg-blue-950/40 dark:text-blue-300"><Camera className="h-5 w-5" /></span><div className="min-w-0"><h3 className="truncate font-bold text-slate-900 dark:text-white">{camera.name}</h3><p className="max-w-full truncate text-sm text-slate-500 dark:text-slate-400">{camera.location || 'Local não informado'} · {camera.host}:{camera.port}</p></div></div><div className="flex min-w-0 max-w-full shrink-0 flex-col items-start gap-1 sm:items-end"><span className={`max-w-full truncate whitespace-nowrap rounded-full px-2.5 py-1 text-xs font-bold ${camera.gatewayStatus === 'ONLINE' ? 'bg-emerald-100 text-emerald-800' : camera.gatewayStatus === 'OFFLINE' ? 'bg-red-100 text-red-800' : 'bg-amber-100 text-amber-800'}`}>{gatewayLabel(camera)}</span>{gateway && <span className={`max-w-full truncate whitespace-nowrap text-[11px] font-semibold ${gateway.relayStatus === 'ONLINE' ? 'text-emerald-600 dark:text-emerald-400' : 'text-slate-500 dark:text-slate-400'}`}>{relayLabel(gateway)}</span>}</div></div><dl className="mt-4 grid grid-cols-1 gap-3 text-sm sm:grid-cols-2"><div><dt className="text-slate-500 dark:text-slate-400">Tipo</dt><dd className="font-semibold text-slate-800 dark:text-slate-200">{camera.deviceType === 'NVR' ? `NVR · canal ${camera.channel}` : 'Câmera IP'}</dd></div><div><dt className="text-slate-500 dark:text-slate-400">Perfil</dt><dd className="font-semibold text-slate-800 dark:text-slate-200">{camera.streamProfile === 'SUB' ? 'Substream' : 'Mainstream'}</dd></div></dl><div className="mt-4 flex min-w-0 flex-wrap gap-2"><button type="button" aria-label="Visualizar câmera" title="Visualizar câmera" onClick={() => setPlayerCamera(camera)} disabled={!camera.active} className="inline-flex h-10 w-10 items-center justify-center rounded-lg bg-blue-700 p-2 text-sm font-bold text-white disabled:cursor-not-allowed disabled:opacity-50"><Radio className="h-4 w-4" /></button><button type="button" aria-label="Testar conexão" title="Testar conexão" onClick={() => void testCamera(camera)} className="inline-flex h-10 w-10 items-center justify-center rounded-lg border border-slate-300 p-2 text-sm font-semibold text-slate-700 dark:border-slate-700 dark:text-slate-200"><Gauge className="h-4 w-4" /></button><button type="button" aria-label="Editar câmera" title="Editar câmera" onClick={() => setFormCamera(camera)} className="inline-flex h-10 w-10 items-center justify-center rounded-lg border border-slate-300 p-2 text-sm font-semibold text-slate-700 dark:border-slate-700 dark:text-slate-200"><Edit3 className="h-4 w-4" /></button><button type="button" aria-label={camera.active ? 'Desativar câmera' : 'Ativar câmera'} title={camera.active ? 'Desativar câmera' : 'Ativar câmera'} onClick={() => void toggleCamera(camera)} className="inline-flex h-10 w-10 items-center justify-center rounded-lg border border-amber-300 p-2 text-sm font-semibold text-amber-800 dark:border-amber-800 dark:text-amber-300">{camera.active ? <Power className="h-4 w-4" /> : <PowerOff className="h-4 w-4" />}</button><button type="button" aria-label="Excluir câmera" title="Excluir câmera" onClick={() => void deleteCamera(camera)} className="inline-flex h-10 w-10 items-center justify-center rounded-lg border border-red-200 p-2 text-sm font-semibold text-red-700 dark:border-red-900 dark:text-red-300"><Trash2 className="h-4 w-4" /></button></div></article>; })}</div>}
        </section>
      </div>
      ...
      {formCamera !== undefined && currentInstitutionId && <CameraForm institutionId={currentInstitutionId} camera={formCamera} gatewayIdOverride={createdGatewayId} gateways={gateways} onClose={() => { clearNewCameraDraft(currentInstitutionId); setFormCamera(undefined); }} onSaved={() => { clearNewCameraDraft(currentInstitutionId); setFormCamera(undefined); setNotice('Câmera salva com sucesso.'); }} />}
      {playerCamera && <CameraPlayer camera={playerCamera} streamSession={streamSession} onClose={() => setPlayerCamera(null)} />}
    </main>
  );
}
