-- Qualifica attempt_id nas consultas para não colidir com a coluna de retorno
-- da função submit_learning_attempt.
...
begin
  ...
  delete from public.learning_answers la
  where la.attempt_id = v_attempt;
  ...
  select coalesce(sum(la.points_awarded), 0)::integer
    into v_score
  from public.learning_answers la
  where la.attempt_id = v_attempt;
