-- Allow schools to identify curriculum items that complement the weekly load.
-- Complementary items remain available for academic use, but are not a hard
-- generation requirement while they have no qualified teacher.

begin;
alter table public.curriculum_template_items
  add column if not exists is_complementary boolean not null default false;
alter table public.class_curriculum_items
  add column if not exists is_complementary boolean not null default false;
...
begin
  ...
  foreach class_item in array p_class_ids loop
    ...
    for template_item in select * from public.curriculum_template_items where template_id = p_template_id and institution_id = p_institution_id and active is true loop
      ...
      values (p_institution_id, class_item, template_item.subject_id, template_item.weekly_lessons, template_item.lesson_duration_minutes, template_item.is_complementary, false, true)
      on conflict (class_id, subject_id) do update set weekly_lessons = excluded.weekly_lessons, lesson_duration_minutes = excluded.lesson_duration_minutes, is_complementary = excluded.is_complementary, active = true, needs_review = false, updated_at = now();
