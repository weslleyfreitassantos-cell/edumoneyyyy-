...
async function createFixture(): Promise<Fixture> {
  ...
  const attendanceSessionA = (await insertOne(service, 'attendance_sessions', { institution_id: institutionA, subject_offering_id: offeringA, session_date: '2026-05-04', starts_at: '08:00', ends_at: '08:50', status: 'CLOSED', created_by: actors['teacher-a'].id })).id;
  const attendanceSessionB = (await insertOne(service, 'attendance_sessions', { institution_id: institutionB, subject_offering_id: offeringB, session_date: '2026-05-04', starts_at: '08:00', ends_at: '08:50', status: 'CLOSED', created_by: actors['teacher-b'].id })).id;
  const attendanceRecordA = (await insertOne(service, 'attendance_records', { institution_id: institutionA, attendance_session_id: attendanceSessionA, student_id: studentA, status: 'PRESENT', recorded_by: actors['teacher-a'].id })).id;
  await insertOne(service, 'attendance_records', { institution_id: institutionB, attendance_session_id: attendanceSessionB, student_id: studentB, status: 'PRESENT', recorded_by: actors['teacher-b'].id });
